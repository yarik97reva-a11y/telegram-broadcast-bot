...
async def add_chat_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(
        ...
        "Чтобы добавить чат:\n"
        "1. Добавьте этого бота в ваш чат\n"
        "2. Сделайте бота <b>администратором</b> (обязательно!)\n"
        "3. Узнайте ID чата через @RawDataBot\n"
        "4. Отправьте мне ID чата\n\n"
        "⚠️ <b>Важно:</b> Бот будет отправлять рассылку в <b>личные сообщения</b> участникам чата.\n\n"
...
async def add_chat_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем доступ к чату и получаем информацию
    try:
        # Получаем информацию о чате
        chat = await context.bot.get_chat(chat_id)

        # Проверяем что бот является членом чата
        bot_member = await context.bot.get_chat_member(chat_id, context.bot.id)

        if bot_member.status not in ['administrator', 'member']:
            await update.message.reply_text(
                "❌ Бот не найден в этом чате.\n"
        ...
        # Добавляем в БД
        db.add_target_chat(chat_id, chat_name, chat.type)

        # Отправляем приветственное сообщение в чат с просьбой зарегистрироваться
        keyboard = [[InlineKeyboardButton("📝 Зарегистрироваться", url=f"https://t.me/{context.bot.username}?start=register")]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        ...
        await context.bot.send_message(
            ...
            text=(
                "👋 <b>Привет!</b>\n\n"
                "Я бот для рассылок. Чтобы получать рассылки в личные сообщения, "
                "нажмите кнопку ниже и зарегистрируйтесь.\n\n"
                "⚠️ Без регистрации рассылки вам не будут приходить!"
            ),
            reply_markup=reply_markup,
            parse_mode="HTML"
        )
        ...
        await update.message.reply_text(
            ...
            f"📢 Участникам отправлено приглашение зарегистрироваться.\n"
            f"После регистрации они смогут получать рассылки в личные сообщения.",
            parse_mode="HTML"
        )
